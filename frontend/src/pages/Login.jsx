...
const Login = () => {
    ...
    const manejarEnvio = async (e) => {
        ...
        try {
            if (estaRegistrando) {
                // Registro de usuario
                await clienteAxios.post('/auth/registro', { usuario, contrasena });
                setMensaje('Usuario registrado con éxito. Ahora puedes iniciar sesión.');
                setEstaRegistrando(false);
                setContrasena('');
            } else {
                // Inicio de sesión
                const res = await clienteAxios.post('/auth/login', { usuario, contrasena });

                if (res.data.requiere2FA) {
                    localStorage.setItem('idUsuarioTemporal', res.data.idUsuario);
